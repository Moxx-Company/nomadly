...
const user = {
  ...
  vpsPlans: '🔧 अपने VPS को प्रबंधित करें',
...
<strong>• 📅 मासिक –</strong> $${generateBilingCost(vpsDetails, 'monthly')} → 10% की बचत करें
<strong>• 📅 तिमाही –</strong> $${generateBilingCost(vpsDetails, 'quaterly')} → 15% की बचत करें
<strong>• 📅 वार्षिक –</strong> $${generateBilingCost(vpsDetails, 'annually')} → 20% की बचत करें
`,
  planTypeMenu: vpsOptionsOf(vpsPlanMenu),
  hourlyBillingMessage: `⚠️ प्रति घंटा बिलिंग के लिए $${VPS_HOURLY_PLAN_MINIMUM_AMOUNT_PAYABLE} की वापसी योग्य जमा राशि आवश्यक है। (यह निर्बाध सेवा सुनिश्चित करता है और यदि अप्रयुक्त रहता है तो वापस कर दिया जाता है।)
...
${list
  .map(
    config =>
      `<strong>• ${config.name} -</strong>  $${config.monthlyPrice}/महीना ($${config.hourlyPrice}/घंटा) – ${config.specs.vCPU} vCPU, ${config.specs.RAM}GB RAM, ${config.specs.disk}GB डिस्क`,
  )
  .join('\n')}`,
  ...
  askForCoupon: `🎟️ एक कूपन कोड दर्ज करें ताकि छूट प्राप्त कर सकें, या इस चरण को छोड़ें।`,
  ...
  askVpsOS: `💡 डिफ़ॉल्ट ऑपरेटिंग सिस्टम: उबंटू (लिनक्स) (यदि कोई चयन नहीं किया जाता है)।
💻 एक ऑपरेटिंग सिस्टम चुनें (Windows Server के लिए $${VPS_WINDOWS_SERVER_OS_PRICE}/महीना अतिरिक्त शुल्क)।  
...
<strong>• Windows Server –</strong> Windows-आधारित अनुप्रयोगों के लिए (+$${VPS_WINDOWS_SERVER_OS_PRICE}/महीना)`,
...
${list.map(item => `${name == 'whm' ? `<strong>• ${item.name} - </strong>` : ''}${item.label}`).join('\n')}`,
  ...
  trialPanelWarning: panel =>
    `ℹ️ ${panel} परीक्षण स्वचालित रूप से $${VPS_CPANEL_PRICE}/माह में नवीनीकृत हो जाता है जब तक कि इसे रद्द नहीं किया जाता।`,
...
<strong>•📅 बिलिंग चक्र (${vpsPlans[vpsDetails.plan]} योजना) –</strong> $${vpsDetails.plantotalPrice}
<strong>•💻 OS लाइसेंस (${vpsDetails.os ? vpsDetails.os.name : 'चयनित नहीं'}) –</strong> $${vpsDetails.selectedOSPrice}
...
<strong>•🎟️ कूपन छूट –</strong> -$${vpsDetails.couponDiscount}
<strong>•🔄 स्वचालित नवीनीकरण –</strong>  ${
    vpsDetails.plan === 'hourly' || vpsDetails.autoRenewalPlan ? '✅ सक्षम' : '❌ अक्षम'
  }

<strong>💰 कुल :</strong> $${vpsDetails.totalPrice}
...
${
  vpsDetails.plan === 'hourly'
    ? `कृपया ध्यान दें, घंटे के हिसाब से योजना के लिए आपको कम से कम ${VPS_HOURLY_PLAN_MINIMUM_AMOUNT_PAYABLE}$ का भुगतान करना होगा। बाकी राशि आपके वॉलेट में जमा हो जाएगी।`
    : ''
}
...
<strong>🔑 लॉगिन विवरण:</strong>
  ...
  <strong>• पासवर्ड:</strong> ईमेल द्वारा भेजा गया (तुरंत बदलें).
...
${price}$ आपके वॉलेट से काटे गए हैं।`,
  ...
  bankPayVPS: (
    priceNGN,
    plan,
  ) => `कृपया “भुगतान करें” पर क्लिक करके ${priceNGN} NGN भेजें। एक बार लेन-देन की पुष्टि हो जाने पर, आपको तुरंत सूचित किया जाएगा और आपकी ${
    vpsPlans[plan]
  } VPS योजना सक्रिय हो जाएगी।
${
  plan === 'hourly'
    ? `कृपया ध्यान दें, घंटे के हिसाब से योजना के लिए आपको कम से कम ${VPS_HOURLY_PLAN_MINIMUM_AMOUNT_PAYABLE}$ का भुगतान करना होगा। बाकी राशि आपके वॉलेट में जमा हो जाएगी।`
    : ''
...
कृपया कुछ समय बाद पुनः प्रयास करें।`,
  confirmDeleteVpstext: name =>
    `⚠️ चेतावनी: इस VPS (${name}) को हटाना स्थायी है, और सभी डेटा खो जाएगा। क्या आप जारी रखना चाहते हैं?`,
...
कृपया कुछ समय बाद पुनः प्रयास करें।`,
  ...
  newVpsPlanBtn: plan => {
    ...
    return `🔼 ${newPlan.newplan} पर उन्नत करें`
  },
  upgradeVpsPlanMsg: `⚙️ अपने VPS संसाधनों को स्केल करने के लिए एक नई योजना चुनें।
...
${Object.values(vpsToUpgradePlan)
  .map(
    planDetails =>
      `<strong>• ${planDetails.current} ➡ ${planDetails.newplan} –</strong> $${planDetails.pricePerMonth}/महीना ($${planDetails.pricePerHour}/घंटा)`,
  )
  .join('\n')}
...
💰 बिलिंग नोटिस: आपका वर्तमान योजना अप्रयुक्त दिनों के लिए क्रेडिट किया जाएगा, और नया दर बिलिंग चक्र के शेष भाग के लिए लागू होगा (प्रोरेटेड समायोजन)।`,
  ...
  alreadyHighestDisk: `⚠️ आप पहले से ही उच्चतम उपलब्ध डिस्क (Extreme Persistent Disk) पर हैं। आगे कोई उन्नयन संभव नहीं है।`,
...
${upgrades
  .map(
    val =>
      `<strong>• ${val.currentName} (${val.currentType}) ➡ ${val.upgradeName} (${val.upgradeType}) –</strong> +$${val.pricePerMonth}/महीना`,
  )
  .join('\n')}
...
💰 बिलिंग नोटिस: यदि उन्नयन मध्य चक्र में लागू किया जाता है, तो आपके वर्तमान बिलिंग अवधि के अप्रयुक्त भाग के लिए प्रोरेटेड समायोजन लागू किया जाएगा।`,
  upgradePlanSummary: (newData, vpsDetails) => `<strong>📜 आदेश सारांश:</strong>
...
<strong>✅ आदेश के साथ आगे बढ़ें?</strong>`,
  upgradeDiskSummary: (newData, vpsDetails) => `<strong>📜 आदेश सारांश:</strong>

<strong>• VPS आईडी: </strong> ${vpsDetails.name}
<strong>• पुराना डिस्क प्रकार: </strong> ${vpsDetails.diskType}
<strong>• नया डिस्क प्रकार: </strong> ${newData.newDisk}
<strong>• नई बिलिंग दर: </strong> $${newData.totalPrice}/महीना  (प्रोरेटेड समायोजन लागू)

<strong>✅ आदेश के साथ आगे बढ़ें?</strong>`,

  vpsSubscriptionData: vpsData => `<strong>🗂️ आपकी सक्रिय सदस्यताएँ:</strong>

<strong>• VPS ${vpsData.name} </strong>– समाप्ति (स्वचालित नवीनीकरण: ${vpsData.autoRenewable ? 'सक्रिय' : 'निष्क्रिय'})
<strong>• नियंत्रण पैनल ${vpsData?.cPanel ? vpsData.cPanel + ' - ' : ': चयनित नहीं'} </strong> ${
    vpsData?.cPanel ? 'नवीकरण' : ''
  } `,

  manageVpsSubBtn: '🖥️ VPS सदस्यता प्रबंधित करें',
  manageVpsPanelBtn: '🛠️ नियंत्रण पैनल सदस्यता प्रबंधित करें',

  vpsSubDetails: data => `<strong>📅 VPS सदस्यता विवरण:</strong>

<strong>• VPS आईडी:</strong> ${data.name}
<strong>• योजना:</strong> ${data.plan}
<strong>• वर्तमान समाप्ति तिथि:</strong> [तारीख]
<strong>• स्वचालित नवीनीकरण:</strong> ${data.autoRenewable ? 'सक्रिय' : 'निष्क्रिय'}`,
  ...
  bankPayVPSUpgradePlan: (priceNGN, vpsDetails) =>
    `कृपया “भुगतान करें” पर क्लिक करके ${priceNGN} NGN जमा करें। एक बार लेन-देन की पुष्टि हो जाने पर, आपको तुरंत सूचित किया जाएगा, और आपका VPS योजना ${vpsDetails.newConfig.name} के साथ सुचारू रूप से सक्रिय हो जाएगा।`,

  bankPayVPSUpgradeDisk: (priceNGN, vpsDetails) =>
    `कृपया “भुगतान करें” पर क्लिक करके ${priceNGN} NGN जमा करें। एक बार लेन-देन की पुष्टि हो जाने पर, आपको तुरंत सूचित किया जाएगा, और आपका VPS योजना नए डिस्क प्रकार ${vpsDetails.newDisk} के साथ सुचारू रूप से सक्रिय हो जाएगा।`,
...
कृपया कुछ समय बाद पुनः प्रयास करें।`,
  ...
  selectSSHKeyToDownload: '🗂️ डाउनलोड करने के लिए SSH कुंजी चुनें:',
  disabledAutoRenewal:
    data => `⚠️ स्वतः-नवीनीकरण अक्षम कर दिया गया है। यदि इसे मैन्युअल रूप से नवीनीकृत नहीं किया गया तो आपका VPS [Date] को समाप्त हो जाएगा।  
✅ स्वतः-नवीनीकरण सफलतापूर्वक अक्षम कर दिया गया है।`,
  enabledAutoRenewal: data => `✅ स्वतः-नवीनीकरण सक्षम। आपका VPS [Date] को स्वचालित रूप से नवीनीकृत हो जाएगा।`,

  payNowBtn: '✅ अभी भुगतान करें',

  vpsChangePaymentRecieved: `✅ भुगतान सफल! आपका VPS सेटअप किया जा रहा है। विवरण जल्द ही उपलब्ध होंगे।`,

  bankPayVPSRenewPlan: priceNGN =>
    `कृपया नीचे "भुगतान करें" बटन पर क्लिक करके ${priceNGN} NGN भेजें। एक बार लेन-देन की पुष्टि हो जाने के बाद, आपको सूचित किया जाएगा और आपका VPS प्लान निर्बाध रूप से सक्रिय और नवीनीकृत हो जाएगा।`,

  unlinkCpanelConfirmed: data =>
    `✅ कंट्रोल पैनल ${data.cPanel} को VPS ${data.name} से सफलतापूर्वक अनलिंक कर दिया गया है।`,
}
